...
In this chapter, we address the challenge of crosstalk by harnessing the power
of quantum optimal control. Our approach involves the development and implementation of robust ancilla control with respect to detuning due to occupations in the cavity modes.
...
In the following, I introduce an alternative optimization-based procedure to design detuning-robust control for transmon $X_{\theta}$ gates and benchmark its performance.
...
\begin{equation}
\label{eq:H_trans}
    H_\text{trans}(t) = -\frac{K_b}{2} b^{\dagger 2} b^2 + \varepsilon_I(t)(b^\dagger+b)+i\varepsilon_Q(t)(b^\dagger-b)+\delta b^\dagger b.
\end{equation}
Here $b$ and $b^\dagger$ are the transmon ladder operators in the same convention as mode~$b$ in Sec.~\ref{sec:parametric_two_mode_control} (cf.\ Eq.~\eqref{eq:Hnl_bare}), with $K_b>0$ the corresponding Kerr coefficient. The quantities $\varepsilon_I(t)$ and $\varepsilon_Q(t)$ describe control envelopes in the $I$ and $Q$ quadratures, and $\delta$ denotes the difference between the drive frequency and the unknown shifted qubit frequency.%
\nomenclature{$\varepsilon_I$, $\varepsilon_Q$}{Control envelopes in the $I$ and $Q$ quadratures}%
\nomenclature{$K_b$}{Transmon Kerr coefficient (as in Eq.~\eqref{eq:Hnl_bare})}%
The closed-system gate infidelity follows 
\begin{equation}\label{eq:fid_single}
\mathcal{I}_\text{c}(\delta) = 1-\frac{1}{4}\left|\Tr\left[P_{0,1}U_\text{target}^\dagger U_\delta(t_\text{g})\right]\right|^2,
\end{equation}
where $P_{0,1}$ is a projector onto the computational subspace formed by the lowest two eigenstates. Here, $U_\text{target}$ is the target unitary on the transmon, and $U_\delta(t_\text{g})=\mathcal{T}\exp[-i\int_0^{t_\text{g}} \mathrm{d}\tau \, H_\text{trans}(\tau)]$ is the actual unitary achieved over a gate duration $t_\text{g}$, where $\mathcal{T}$ is the time-ordering operator. 
...
For the closed-system optimization, I use Gradient Ascent Pulse Engineering (GRAPE), a method widely employed for optimal control in various contexts~\cite{nmrkhaneja2005optimal,lu2023optimalgrape,koch2022quantumqocreview}. The gradients required in GRAPE are calculated by using automatic differentiation, thereby eliminating the need for manually coding the analytical gradients of the cost function~\cite{adscalingbaydin2014automatic,pacleung2017speedup,abdelhafez2019gradient}. The initial pulse ansatz is
\begin{align}
...
\nonumber    \varepsilon_Q^\text{initial}(t) & = \dot{\varepsilon}_I^\text{initial}(t) / K_b,
\end{align}
where $\theta$ is the rotation angle for the target $X_\theta$ gate ($\theta=\pi,\pi/2$ in this study). This ansatz for the $I$-quadrature control has been used to obtain robust control for two-level systems~\cite{pasini2009optimizedinitialpulse}. The initial guess for the $Q$-quadrature is inspired by the DRAG pulse scheme~\cite{Motzoi2009} to suppress leakage. 
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{chapter5/figures/fig_qoc.pdf}
    \caption[Optimization results for the $X_\pi$ gate.]{
    Optimization results for the $X_\pi$ gate. 
...
[Parameters: $\delta_i/2\pi\in\{\pm 1,\pm 3,\pm 4,\pm 7,\pm 10\}$\,MHz, $K_b/2\pi=200$\,MHz, $T_1=50$\,$\mu$s, $T_\phi=50$\,$\mu$s.]}
    \label{fig:fig_qoc}
\end{figure}

I benchmark the optimization procedure for a transmon with typical parameters: $K_b/2\pi=200$\,MHz, $T_1=50$\,$\mu$s, and $T_\phi=50$\,$\mu$s. The optimization results for the $X_\pi$ gate are shown in Fig.~\ref{fig:fig_qoc}. 
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{chapter5/figures/fig_qoc_qctrl.pdf}
    \caption[Comparison of optimization results between Q-CTRL and QOC for the $X_{\pi}$ gate.]{
Comparison of optimization results between Q-CTRL and QOC for the $X_{\pi}$ gate.
