...
\caption{Decision trees for selecting the optimal numerical strategy. (a)~Choosing among three numerical methods for evaluating gradients: full automatic differentiation, semi-automatic differentiation, hard-coded analytical gradients. (Here, $\mu\sim\Theta(d)$ is assumed; for different scaling see Table~\ref{tab:scalingtable}.) (b)~Determining the appropriate strategy for evaluating propagator-state products.}
