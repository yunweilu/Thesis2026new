...
\begin{figure}[H]
  \centering
\includegraphics[width=0.9\linewidth]{chapter2/figures/circuit.pdf}
...
\begin{equation}
    H_{0} = \frac{Q^{2}}{2C} + \frac{\Phi^{2}}{2L}.
\end{equation}
...
\begin{equation}
    \Phi = \Phi_{\mathrm{zpf}}(a+a^{\dagger}),
    \qquad
    Q = -iQ_{\mathrm{zpf}}(a-a^{\dagger}),
\end{equation}
with $[a,a^{\dagger}] = 1$. Substituting these expressions into $H_{0}$ yields
\begin{equation}
    H_{0}
    =
    \omega_{0}\left(a^{\dagger}a+\frac{1}{2}\right),
\end{equation}
...
\begin{equation}
    \ket{\tilde \psi(t)}
    =
    D^\dagger[\alpha(t)] \ket{\psi(t)},
    \qquad
    D(\alpha)
    =
    \exp\!\left(\alpha a^\dagger - \alpha^* a\right).
\end{equation}
Substituting this transformation into the Schr\"odinger equation and requiring that the terms linear in $a$ and $a^\dagger$ cancel yields the equation of motion
\begin{equation}
    \dot \alpha(t)
    =
...
This flux tunability is also the basis of parametric control. By modulating $\Phi_{\mathrm{ext}}$ in time, one can generate sideband interactions between the transmon and coupled cavity modes, including beam-splitter, as discussed in Section~\ref{sec:parametric_two_mode_control}.
...
Now couple the transmon capacitively to a single cavity mode of frequency \(\omega_c\), described by the annihilation operator \(a\) [Fig.~\ref{fig:circuit_elements}(e)]. In the transmon eigenbasis, the uncoupled Hamiltonian is
\begin{equation}
    H_0
    =
    \omega_c a^\dagger a
...
\begin{equation}
    H_{\mathrm{disp}}^{(ge)}
    =
    (\omega_c+\bar{\chi})a^\dagger a
    +
    \frac{\tilde{\omega}_q}{2}\sigma_z
    +
    \frac{\chi}{2}a^\dagger a\,\sigma_z.
...
\begin{equation}
    D[\alpha(t)] = \exp\!\left[\alpha(t)a^\dagger-\alpha^*(t)a\right],
\end{equation}
...
\begin{equation}
    ...
    \frac{\chi}{2}
    \left(
        \alpha(t)a^\dagger + \alpha^*(t)a
    \right)\sigma_z
    +
    \frac{\chi}{2}|\alpha(t)|^2 \sigma_z
    +
    \frac{1}{2}
    \left[
        \Omega^*(t)\sigma_-
...
To implement a target cavity unitary \(U_{\mathrm{cav}}\), one works in a truncated Fock space
\begin{equation}
...
\end{equation}
which describes the effective action of the sequence on the cavity when the qubit begins and ends in \(\ket{g}\). The synthesis condition in Eq.~\eqref{eq:ecd_synthesis_condition} is satisfied if and only if $M_N = U_{\text{cav}}$ up to a global phase, which motivates minimizing the following infidelity
\begin{equation}
    C_{\mathrm{gate}}
    =
    1
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\linewidth]{chapter2/figures/fig_ecd.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{chapter2/figures/twomode.pdf}
...
We consider an FTT capacitively coupled to two cavities, as shown in Fig.~\ref{fig:twomode}. Let \(a\) and \(c\) denote the two linear cavity modes, and let \(b\) denote the FTT mode. The circuit Hamiltonian is
\begin{equation}
    H
    =
    H_{\mathrm{lin}}(t)
...
\begin{equation}
    ...
    \omega_c c^\dagger c
    +
    \omega_b\!\left(\Phi_{\mathrm{ext}}(t)\right)b^\dagger b
    +
...
\begin{equation}
    ...
    -
    E_C .
    \label{eq:omegab_flux}
\end{equation}
Expanding around the dc bias point gives
\begin{equation}
    \omega_b\!\left(\Phi_{\mathrm{ext}}(t)\right)
    \approx
    \bar\omega_b
    +
    \delta\omega_b \cos(\omega_d t+\phi_d),
    \label{eq:omegab_expand}
\end{equation}
where
\begin{equation}
    \bar\omega_b = \omega_b(\Phi_{\mathrm{dc}}),
    \qquad
    \delta\omega_b
    =
    \left.
...
\begin{equation}
    ...
    \mu=a,b,c,
\end{equation}
where \(d_\nu^{(\mathrm{N})}\) is the annihilation operator for normal mode~\(\nu\), one obtains
\begin{equation}
    H^{(0)}
    =
    \tilde\omega_a a_{\mathrm N}^\dagger a_{\mathrm N}
    +
    \tilde\omega_b b_{\mathrm N}^\dagger b_{\mathrm N}
    +
    \tilde\omega_c c_{\mathrm N}^\dagger c_{\mathrm N} .
    \label{eq:H0_diag}
\end{equation}
Here \(u_{\mu\nu}\) is an element of the inverse diagonalization matrix, expressing bare mode \(\mu\) in terms of normal mode \(\nu\).

In the parameter regime of interest, two of these normal modes remain predominantly cavity-like and one remains predominantly transmon-like. For notational simplicity, we henceforth drop the subscript \(\mathrm N\) and write \(a\), \(b\), and \(c\) for the dressed normal modes unless stated otherwise. In this basis,
\begin{equation}
    b
    =
    \sum_{\nu=a,b,c}
    u_{b\nu}\,d_\nu,
\end{equation}
where \(d_\nu\) denotes the annihilation operator for normal mode~\(\nu\) (so \(d_a\equiv a\), \(d_b\equiv b\), and \(d_c\equiv c\)), so that
\begin{equation}
    b^\dagger b
    =
    \sum_{\mu,\nu=a,b,c}
...
\(
\tilde\omega_a a^\dagger a
+
\tilde\omega_c c^\dagger c
\),
