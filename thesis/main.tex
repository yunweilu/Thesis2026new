%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Document type, global settings, and packages
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\documentclass[12pt]{report}   %12 point font for Times New Roman
\usepackage{graphicx}  %for images and plots
% ./ = compile from thesis/; thesis/ = compile from repo root (e.g. Overleaf)
\graphicspath{{./}{thesis/}}
\usepackage[letterpaper, left=1in, right=1in, top=1in, bottom=1in, includefoot,includehead,heightrounded]{geometry}  %margins; include* forces the page numbers and other header/footer within margin space. Looks gross but required.
\usepackage{setspace}  %use this package to set linespacing as desired
\usepackage{float}
\usepackage{times}  %set Times New Roman as the font
% expansion=false avoids \showhyphens patch (kernel vs. microtype mismatch on newer LaTeX)
\usepackage[protrusion=true,expansion=false]{microtype}
\usepackage[explicit]{titlesec}  %title control and formatting
\usepackage[titles]{tocloft}  %table of contents control and formatting
% Allow TOC/LOF/LOT entries to break before the page number (reduces overfull \hbox)
\makeatletter
\renewcommand{\@tocrmarg}{2.55em plus 1fil}
\makeatother
% \usepackage[backend=bibtex, sorting=none, bibstyle=ieee]{biblatex}  %reference manager
\usepackage[bookmarks=true, hidelinks]{hyperref}
\usepackage{url}
\def\UrlBreaks{\do\/\do-\do_}
\usepackage{appendix}  %for appendices
\usepackage{import}  % \subimport: paths relative to this file (Overleaf CWD != local)
\usepackage{rotating}  %for rotated, landscape images
\usepackage[normalem]{ulem}  %for italicized text
\usepackage{xcolor}
\usepackage{chemformula} % Formula subscripts using \ch{}
\usepackage{siunitx}
% physics also defines \qty; we restore siunitx's \qty below. Silence the stock warning.
\ExplSyntaxOn
\msg_redirect_name:nnn { siunitx } { physics-pkg } { none }
\ExplSyntaxOff
\usepackage{amssymb}
\usepackage{amsfonts}
% List of Symbols: define entries in symbols.tex only (\nomenclature)
\usepackage[intoc]{nomencl}
\makenomenclature
% ---------------------------------------------------------------------------
% Added stuff to the template 
% ---------------------------------------------------------------------------
\usepackage[numbers,sort&compress]{natbib}
\usepackage{array}
\usepackage{multirow}  %multi-row tables
%
% Supplementary materials still show up in list of figures/tables
\usepackage{newfloat}
\DeclareFloatingEnvironment[name={Figure S}, fileext=lof]{suppfigure}
\DeclareFloatingEnvironment[name={Table S}, fileext=lot]{supptable}
% /Supplementary materials
%
% Top right page number (Fancy)
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhf{}
\renewcommand{\headrulewidth}{0pt}
\fancyhead[R]{\thepage}
\setlength{\headheight}{14.49998pt}
\addtolength{\topmargin}{-2.49998pt}
% /Top right page number
%
% 1st page should be fancy
\makeatletter
\renewcommand\chapter{\if@openright\cleardoublepage\else\clearpage\fi
                    \thispagestyle{fancy}%
                    \global\@topnum\z@
                    \@afterindentfalse
                    \secdef\@chapter\@schapter}
\makeatother
% /1st page should be fancy
%
\usepackage{tocvsec2}  % approval page is not a chapter. Remove from toc by excluding all \chapter* from toc.
%
% 
% DEBUGGING: show page margins
% \usepackage{showframe}  % turn off when done
% ---------------------------------------------------------------------------
% Math and physics packages (from integrated papers)
% ---------------------------------------------------------------------------
\usepackage{amsmath}
\usepackage{mathtools}
\usepackage{physics}
\usepackage{stmaryrd}
\SetSymbolFont{stmry}{bold}{U}{stmry}{m}{n}
\usepackage{mathbbol}
\usepackage{cleveref}
\usepackage{colortbl}
\usepackage{listings}


\DeclareMathOperator{\cc}{c.c.}
\DeclareMathOperator{\Real}{Re}

\newcommand{\dket}[1]{|#1\rangle}
\newcommand{\dbra}[1]{\langle#1|}
\renewcommand{\norm}[1]{\left|\left|#1\right|\right|}
\newcommand{\dbraket}[2]{\langle#1|#2\rangle}
\newcommand{\parenthesis}[1]{\left(#1\right)}
\newcommand{\ceiling}[1]{\lceil#1\rceil}
\newcommand{\floor}[1]{\lfloor#1\rfloor}
\newcommand{\be}{\begin{equation}}
\newcommand{\ee}{\end{equation}}
\newcommand{\kett}[1]{|\,#1\rangle\!\rangle}
\newcommand{\bbra}[1]{\langle\!\langle#1\,|}
\newcommand{\bbrakett}[2]{\langle\!\langle#1\,|\,#2\rangle\!\rangle}
\newenvironment{widetext}{}{}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Bibliography (APS / revtex4-2 style via natbib + apsrev4-2)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%
% Start of Document
%%%%%%%%%%%%%%%%%%%%%%
% physics defines \qty as matrix shorthand; restore siunitx's \qty for units.
\AtBeginDocument{\RenewCommandCopy\qty\SI}

\begin{document}
\doublespacing  %set line spacing

% Front matter uses roman numerals
\pagenumbering{roman}
\setcounter{page}{1}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Title Page
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\input{titlePage.tex}
\currentpdfbookmark{Title Page}{titlePage}  %add PDF bookmark for this page

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%copyright page
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{copyright.tex}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is the abstract 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\addcontentsline{toc}{chapter}{Abstract}
\input{abstract.tex}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Acknowledgements
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\addcontentsline{toc}{chapter}{Acknowledgements}

\input{acknowledgements.tex}

%\addtocontents{toc}{\cftpagenumbersoff{chapter}} 
%\currentpdfbookmark{Acknowledgments}{acknowledgments}
%\addtocontents{toc}{\cftpagenumberson{chapter}} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% List of Publications
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\addcontentsline{toc}{chapter}{List of Publications}
\input{publications.tex}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% List of Symbols
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{symbols.tex}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Dedication
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\addcontentsline{toc}{chapter}{Dedication}
\input{dedication.tex}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Table of Contents
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Format for Table of Contents
\renewcommand{\cftchapdotsep}{\cftdotsep}  %add dot separators
\renewcommand{\cftchapfont}{\bfseries}  %set title font weight
\renewcommand{\cftchappagefont}{}  %set page number font weight
\renewcommand{\cftchappresnum}{Chapter }
\renewcommand{\cftchapaftersnum}{:}
\renewcommand{\cftchapnumwidth}{5em}
\renewcommand{\cftchapafterpnum}{\vskip\baselineskip} %set correct spacing for entries in single space environment
\renewcommand{\cftsecafterpnum}{\vskip\baselineskip}  %set correct spacing for entries in single space environment
\renewcommand{\cftsubsecafterpnum}{\vskip\baselineskip} %set correct spacing for entries in single space environment
\renewcommand{\cftsubsubsecafterpnum}{\vskip\baselineskip} %set correct spacing for entries in single space environment
\setcounter{tocdepth}{3}
\setcounter{secnumdepth}{3}

%format title font size and position (this also applys to list of figures and list of tables)
\titleformat{\chapter}[display]
{\normalfont\bfseries\filcenter}{\chaptertitlename\ \thechapter}{0pt}{\MakeUppercase{#1}}

\renewcommand\contentsname{Table of Contents}

\begin{singlespace}
\tableofcontents
\end{singlespace}

\currentpdfbookmark{Table of Contents}{TOC}

\clearpage

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% List of Tables
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\addcontentsline{toc}{chapter}{List of Tables}
\begin{singlespace}
	\setlength\cftbeforetabskip{\baselineskip}  %manually set spacing between entries
	\listoftables
\end{singlespace}

\clearpage


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% List of Figures
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\addcontentsline{toc}{chapter}{List of Figures}
\begin{singlespace}
\setlength\cftbeforefigskip{\baselineskip}  %manually set spacing between entries
\listoffigures
\end{singlespace}

\clearpage

% Main matter uses arabic numerals
\pagenumbering{arabic}
\setcounter{page}{1}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CHAPTERS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%
% formatting
%%%%%%%%%%%%%%%%%%%%%%

% resume page numbering for rest of document
\clearpage

% Adjust chapter title formatting
\titleformat{\chapter}[display]
{\normalfont\bfseries\filcenter}{\MakeUppercase\chaptertitlename\ \thechapter}{0pt}{\MakeUppercase{#1}}  %spacing between titles
\titlespacing*{\chapter}
  {0pt}{0pt}{30pt}	%controls vertical margins on title
  
% Adjust section title formatting
\titleformat{\section}{\normalfont\bfseries}{\thesection}{1em}{#1}

% Adjust subsection title formatting
\titleformat{\subsection}{\normalfont\bfseries}{\thesubsection}{0em}{\hspace{1em}#1}

% Adjust subsubsection title formatting
\titleformat{\subsubsection}{\normalfont\itshape}{\thesubsubsection}{1em}{#1}

% %%%%%%%%%%%%%%%%
% % main text
% %%%%%%%%%%%%%%%%
\input{Introduction.tex}

\input{chapter2/main.tex}
\input{chapter3/main.tex}
\input{chapter4/main.tex}
\input{conclusion.tex}


%%%%%%%%%%%%%%%%
% References
%%%%%%%%%%%%%%%%

\addcontentsline{toc}{chapter}{References}
\begingroup
\singlespacing
\bibliographystyle{apsrev4-2}
\bibliography{ref}
\endgroup

%%%%%%%%%%%%%%%%
% Appendices
%%%%%%%%%%%%%%%%
% \pagestyle{fancy} 
\subimport{appendix/}{main}







\end{document}
