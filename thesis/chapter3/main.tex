...
As shown in the following, for two concrete examples, this method for evaluating $e^A\dket{\Psi}$ can achieve better runtime performance and accuracy than \texttt{expm\_multiply} function~\cite{virtanen2020scipy,al2011computing} implemented in \texttt{SciPy}. The comparisons employ sparse matrix--vector products for two model Hamiltonians that are reused when reporting full GRAPE benchmarks later in this chapter. A driven cavity--transmon system is described in the frame co-rotating with the transmon drive by
\begin{equation}
\label{eq:fockH}
    H_1(t)=\Delta b^\dagger b-\frac{K_b}{2}b^{\dagger 2}b^2+g(cb^\dagger+c^\dagger b)+a_{z}(t)b^\dagger b
    +a_{x}(t)(b+b^\dagger),
\end{equation}
where $c$ and $b$ are the cavity and transmon annihilation operators, $K_b>0$ is the transmon Kerr coefficient, $g$ is the coupling strength, and $\Delta$ is the detuning between transmon and cavity. Three coupled transmons with nearest-neighbor hopping are modeled by
\begin{equation}
\label{eq:gateH}
